...
public class BeatTrackDisplay
   ...
   /** the threshold above which the maximum colour is used in the spectrogram */
	...
	public static final int SHOW_DATA = 0;//4;
	public static final int SHOW_ONSETS = 0;//8;
	public static final int SHOW_AUDIO = 0;//1024;
	public static final int SHOW_SPECTRO = 2048;
	public static final int SHOW_MIDI = 0;//4096;
    ...
    */
	...
	public void resizeSpectroForVisu(int nbLignes){
		int sizeT = spectroLength;
		int sizeF = spectro[0].length;	// 84 (for audio, cf 88? for midi)
		...
		for (int i =0  ; i < sizeT; i++) {
			...
			while(a < sizeF){
				c++;
				newSpec = 0;
				for(;a<trancheVerticale*c  && a<sizeF ;a++){
					newSpec += spectro[i][a];
				}
				newSpec = newSpec/trancheVerticale;
				
				for(;b<trancheVerticale*c  && b<sizeF;b++){
					spectro[i][b] = newSpec;
				}
			}
		}
		...
		for (int i =0  ; i < sizeF; i++) {
			...
			a=0; b=0;c=0;
			if(beatPtr.hasNext())
				selectedBeat = beatPtr.next(); //pour éviter le cas où beatPtr pointe sur null (ce qui arrivera au début)
			selectFirstBeat();
			...
			while(a < sizeT){
				...
				newSpec = newSpec/(myBeat-myPreviousBeat);
				
				for(;b<myBeat && b<sizeT ;b++){
					spectro[b][i] = newSpec;
				}					
			}
		}//fin
